...
setS4Class(SummarizedExperiment)
...
method(names, SummarizedExperiment) <- function(x) {
    NAMES <- x@NAMES
    if (length(NAMES) == 0L)
        NULL
    else
...
{
    methods::new("SummarizedExperiment", object)
}
...
identicalVals <- new_generic("identicalVals", c("x", "y"), function(x, y) S7_dispatch())
...
### efficient.
method(identicalVals, list(class_factor, class_factor)) <- 
    function(x, y)
    {
